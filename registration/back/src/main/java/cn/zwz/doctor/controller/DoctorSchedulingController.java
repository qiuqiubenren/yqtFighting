...
public class DoctorSchedulingController {
    ...
    public Result<Object> addNumber(@RequestParam String doctorId,@RequestParam String date,@RequestParam int step,@RequestParam int number){
        ...
        if(doctor == null) {
            return ResultUtil.error("陪诊员不存在");
        }
        if(number < 1) {
